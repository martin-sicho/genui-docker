...
docker build --build-arg NVIDIA_CUDA_RUNFILE=${NVIDIA_CUDA_RUNFILE} -t ${REPO_PREFIX}/genui-base-cuda:${TAG} -f Dockerfile-base-cuda .
...
docker build -t ${REPO_PREFIX}/genui-gpuworker:${TAG} -f Dockerfile-gpuworker .